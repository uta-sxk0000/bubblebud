import { getVariantOptions, normalizeVariantGroups, products as seedProducts } from "@/lib/products";
import { createAdminSupabase } from "@/lib/supabase/admin";
import { hasServiceConfig } from "@/lib/env";

const TRUSTED_VARIANT_MEDIA_PRODUCT_IDS = new Set([
  "laptop-14",
  "makeup-8",
  "purple-bouquet",
  "small-bouquet",
...
export function normalizeProduct(row) {
  const seedProduct = seedProducts.find((product) => product.id === row.id);
  const useTrustedVariantMedia = seedProduct && TRUSTED_VARIANT_MEDIA_PRODUCT_IDS.has(row.id);
  const seedVariantOptions = seedProduct?.variantOptions || normalizeVariantGroups(seedProduct?.variants || []);
  const gallery = useTrustedVariantMedia && seedProduct?.gallery?.length
    ? seedProduct.gallery
    : row.images?.length
      ? row.images
      : seedProduct?.gallery?.length
        ? seedProduct.gallery
        : ["/assets/hero-arrivals.png"];
  ...
  const variantOptions = useTrustedVariantMedia && seedVariantOptions.length
    ? seedVariantOptions
    : normalizeVariantGroups(row.variants || []);
