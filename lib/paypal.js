...
async function paypalRequest(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(detail || data.message || data.error_description || data.name || "PayPal request failed.");
  }
...
export async function createPayPalOrder({ checkoutIntentId, items, subtotalCents, shippingCents, taxCents = 0, totalCents, currency = "USD" }) {
